...
void main(void){
    ...
    while(1){
        for (unsigned char i=0;i<64;i++){
            for (unsigned char disp=0;disp<8;disp++){
                if (msg[i+disp] == 0x20){
                    pcf8574_escribir(0x00, disp);
                }
                else{
                    pcf8574_escribir(romcharacter[(msg[i+disp]-0x41)], disp);
                }
            }
            _delay((unsigned long)((150)*(48000000UL/4000.0)));
        }
    }
}
