...
configuro:
    bcf TRISD, 5 ;((PORTD) and 0FFh), 5, a como salida
    movlw 0C0H ;Timer0 ON, modo 8bit, 1:2PSC, FOSC/4
    movwf T0CON

loop: movlw 6
 movwf TMR0L ;Cuenta inicial de 6
otro: btfss INTCON, 2 ;Pregunta si ((INTCON) and 0FFh), 2, a=1
 goto otro
 btg LATD, 5 ;Basculamos ((PORTD) and 0FFh), 5, a para que genere la onda cuadrada
 bcf INTCON, 2 ;Bajando la bandera ((INTCON) and 0FFh), 2, a
 goto loop
