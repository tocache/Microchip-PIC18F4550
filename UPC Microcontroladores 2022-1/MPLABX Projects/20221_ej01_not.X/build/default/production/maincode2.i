...
configuro:
    ...
    bcf TRISD, 0 ;((PORTD) and 0FFh), 0, a es una salida
    bsf TRISB, 0 ;((PORTB) and 0FFh), 0, a es una entrada
    bcf TRISD, 3 ;((PORTD) and 0FFh), 3, a es una salida
    bsf TRISB, 3 ;((PORTB) and 0FFh), 3, a es una entrada

loop:
    btfss PORTB, 0 ;Pregunto si ((PORTB) and 0FFh), 0, a es uno
    goto falso ;Falso, salta a etiqueta falso
    bcf LATD, 0 ;Verdad, manda a 0 el ((PORTD) and 0FFh), 0, a
    goto loop2 ;Retorna al inicio (etiqueta loop)
falso:
    bsf LATD, 0 ;Manda a 1 el ((PORTD) and 0FFh), 0, a

loop2: ;Retorna al inicio (etiqueta loop)
    btfss PORTB, 3 ;Pregunto si ((PORTB) and 0FFh), 3, a es uno
    goto falso2 ;Falso, salta a etiqueta falso
    bcf LATD, 3 ;Verdad, manda a 0 el ((PORTD) and 0FFh), 3, a
    goto loop ;Retorna al inicio (etiqueta loop)
falso2:
    bsf LATD, 3 ;Manda a 1 el ((PORTD) and 0FFh), 3, a
    goto loop ;Retorna al inicio (etiqueta loop)
    end not_gate
