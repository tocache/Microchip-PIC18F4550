...
configuro:
    ...
    bsf TRISB, 0 ;Asignamos a ((PORTB) and 0FFh), 0, a como entrada
    bcf TRISD, 0 ;Asignamos a ((PORTD) and 0FFh), 0, a como salida

loop:
    ;aquí va el programa de la aplicación
    btfss PORTB, 0 ;Preguntamos si ((PORTB) and 0FFh), 0, a es uno
    goto falso ;Falso, sato a etiqueta falso
    bcf LATD, 0 ;Verdadero, coloco el ((PORTD) and 0FFh), 0, a a cero
    goto loop ;Salto a etiqueta loop

falso:
    bsf LATD, 0 ;Coloco ((PORTD) and 0FFh), 0, a a uno
    goto loop ;Salto a etiqueta loop
