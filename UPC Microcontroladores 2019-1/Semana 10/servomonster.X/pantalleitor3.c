...
void main(void) {
    /*Configuracion del LCD usando libreria*/
...
//    TRISCbits.RC2 = 0;          //Puerto RC2 como salida (conectado el backlight del LCD)
//    LATCbits.LC2 = 1;           //Encendido el backlight del LCD
    
    /*Configuracion del ADC usando libreria*/
    ADC_CONFIG(0);

    /*Configuracion del CCP1 en PWM*/
    PR2 = 249;                  //Periodo del PWM
    CCPR1L = 50;               //Duty Cycle del PWM 50%
    ...
    while(1){
        /*Seteado del brillo del backlight del LCD*/
        if(PORTBbits.RB0 == 0){
            CCPR1L = 50;               //Duty Cycle del PWM 15% aprox
        }
        else{
            CCPR1L = 200;               //Duty Cycle del PWM 15% aprox            
        }
        
        CURSOR_HOME();
        ESCRIBE_MENSAJE("Leyendo POT:", 11);
        resultadomon = ADC_CONVERTIR();
        DIGITOS(resultadomon);      //Funcion de sacar los digitos de la variable resultadomon
