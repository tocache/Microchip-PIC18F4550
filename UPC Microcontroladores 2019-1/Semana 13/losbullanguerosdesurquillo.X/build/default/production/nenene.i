...
char mensaje_lcd1[] = {"UPC 2019-1"};
char mensaje_lcd2[] = {"Microcontroladores"};

char mensaje_pserial1[] = {"UPC Microcontroladores"};
char mensaje_pserial2[] = {"Menu: Papa a la huancaina con tallarines verdes"};
char mensaje_pserial3[] = {"Jajaja era broma. 'Q' para encender backlight"};
char mensaje_pserial4[] = {"'R' para apagar backlight"};

char mensaje_blighton[] = {"Backlight encendido"};
char mensaje_blightoff[] = {"Backlight apagao"};
char mensaje_error[] = {"Oe has presionao opcion invalida"};
...
void main(void) {
    ...
    while(1){
        CURSOR_HOME();
        ESCRIBE_MENSAJE(mensaje_lcd1, 10);
        POS_CURSOR(2,0);
        ESCRIBE_MENSAJE(mensaje_lcd2, 18);
        if(estado == 1){
            for (int c=0;c<22;c++) {
                TXREG = mensaje_pserial1[c];
...
void __attribute__((picinterrupt(("high_priority")))) RCIsr(void){
    PIR1bits.RC1IF = 0;
    if (RCREG == 0x0D){
        estado = 1;
    }
