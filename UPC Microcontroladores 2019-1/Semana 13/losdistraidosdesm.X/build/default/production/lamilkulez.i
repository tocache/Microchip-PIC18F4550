...
# 15 "lamilkulez.c" 2
# 39 "lamilkulez.c"
char mensaje1[] = {"UPC Mecas Elecs"};
char mensaje2[] = {"Microcontrolas"};

char msghaciapc1[] = {"Bienvenidos al control remoto"};
char msghaciapc2[] = {"Elija opcion: 'Q' para encender el backlight"};
char msghaciapc3[] = {"'R' para apagar el backlight"};
char msghaciapcon[] = {"Backlight encendido"};
char msghaciapcoff[] = {"Backlight apagao"};
char msghaciapcerr[] = {"Joder presionaste tecla invalida"};
...
void SEND_MSG(const char *vectormon,char posicionmon)
{
 for(char i = 0; i<posicionmon; i++)
 {
  TX_CHAR_EUSART(vectormon[i]);
 }
}
...
void main(void){
    ...
    while(1){
        ...
        POS_CURSOR(1,0);
        ESCRIBE_MENSAJE(mensaje1,15);
        POS_CURSOR(2,0);
        ESCRIBE_MENSAJE(mensaje2,14);

        if(estadomon == 1){
            SEND_MSG(msghaciapc1,29);
            ENTERMON();
            SEND_MSG(msghaciapc2,44);
            ENTERMON();
            SEND_MSG(msghaciapc3,28);
            ENTERMON();
            estadomon = 0;
        }
        else if(estadomon == 2){
            SEND_MSG(msghaciapcon,19);
            ENTERMON();
            LATCbits.LC2 = 1;
            estadomon = 0;
        }
        else if(estadomon == 3){
            SEND_MSG(msghaciapcoff,16);
            ENTERMON();
            LATCbits.LC2 = 0;
            estadomon = 0;
        }
        else if(estadomon == 4){
            SEND_MSG(msghaciapcerr,32);
            ENTERMON();
            estadomon = 0;
        }
# 148 "lamilkulez.c"
