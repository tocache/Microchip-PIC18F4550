...
void main(void) {
    ...
    ADCON2 = 0x91;              // 4TAD FOsc/64 y Justificacion a la derecha
    ADCON1 = 0x0E;              // AN0 habilitado
    ADCON0 = 0x01;              //Para encender el modulo A/D
    while(1){
        ADCON0bits.GO = 1;      //Inicie la conversion el A/D en el canal AN0
        while(ADCON0bits.GO);
        conversion = ADRESH << 8;
        conversion = conversion + ADRESL;
        digitos(conversion);
        POS_CURSOR(2,10);
