...
# 3 "maincode.asm" 2
 ...
 goto configura
    ...
    PSECT code
configura:
     bsf TRISB, 0, 0 ;((PORTB) and 0FFh), 4, a es una entrada
     bcf TRISD, 0, 0 ;((PORTD) and 0FFh), 0, a es una salida
inicio:
     btfss PORTB, 0, 0 ;Pregunto si ((PORTB) and 0FFh), 4, a es uno
     goto falso ;Viene aqui cuando es falso, salta a etiqueta falso
     bcf LATD, 0, 0 ;Viene aqui cuando es verdadero, pone ((PORTD) and 0FFh), 0, a a 0
     goto inicio ;Regrese a inicio
falso: bsf LATD, 0, 0 ;((PORTD) and 0FFh), 0, a es 1
     goto inicio
