...
;entrada EQU 000H
; PSECT romData,space=0,class=CONST,delta=1
...
configuracion: movlw 80H
  movwf TRISD ;((PORTD) and 0FFh), 0, a al ((PORTD) and 0FFh), 6, a como salidas
; movlw HIGH tabla_7s;|
; movwf TBLPTRH ; |
; movlw LOW tabla_7s; |
; movwf TBLPTRL ; |TBLPTR apuntando a valores_7s (00500H)
inicio: movf PORTB, w ;Leemos RB y almacenamos el contenido en Wreg
  ...
  call alternativo_PC
  movff 001H, BSR
  ;call sub_tabla_7s ;Lllamada a la subrutina de LUT para decodificar el D7S
  ;movlw 0x5B
  movwf LATD
  goto inicio
  ;Subrutina de la LUT
;sub_tabla_7s: movwf TBLPTRL, a ;Asignamos el valor de entrada a TBLPTRL
; TBLRD* ;Acción de lectura del contenido apuntado por TBLPTR
; movff TABLAT, LATD ;Movemos contenido de TABLAT hacia ((EECON1) and 0FFh), 0, a
; return ;Retorno hacia rutina principal

alternativo_PC: addwf PCL, f
  retlw 3FH
  retlw 06H
  retlw 5BH
  retlw 4FH
