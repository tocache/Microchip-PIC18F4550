...
unsigned char menor20[] = {"Nivel <20C"};
unsigned char mayor40[] = {"Nivel >40C"};
unsigned char normal[] = {"Nivel Norm"};
...
void main(void) {
    ...
    while(1) {
        ...
        if (CMCONbits.C2OUT == 1){
            ...
            serial_siguientelinea();
            POS_CURSOR(1,0);        
            ESCRIBE_MENSAJE(mayor40, 10);
        }
        if  (CMCONbits.C2OUT == 0 && CMCONbits.C1OUT == 0){
            for (unsigned int i=0;i<10;i++){
                TXREG = normal[i];
                while(TXSTAbits.TRMT == 0);
            }
