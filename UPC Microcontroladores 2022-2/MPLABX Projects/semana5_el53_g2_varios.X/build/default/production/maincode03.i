...
configuro:
    movlw 0D0H
    movwf TRISB ;((PORTB) and 0FFh), 5, a, ((PORTB) and 0FFh), 3, a al ((PORTB) and 0FFh), 0, a como salidas
    movlw 80H
    movwf TRISD ;((PORTD) and 0FFh), 6, a al ((PORTD) and 0FFh), 0, a como salidas
    bcf INTCON2, 7 ;((INTCON2) and 0FFh), 7, a habilitadas
    movlw 83H
    movwf T0CON ;TMR0 ON, fosc/4, modo 16bit, psc 1:16
    movlw 0A0H
    movwf INTCON ;((INTCON) and 0FFh), 7, a=1, ((INTCON) and 0FFh), 5, a=1 activamos interr de TMR0
...
Tmr0_ISR:
    bcf INTCON, 2 ;bajamos bandera de desborde de TMR0 (((INTCON) and 0FFh), 2, a=0)
    movlw 0BH
    movwf TMR0H
    movlw 0DCH
    movwf TMR0L ;carga de cuenta inicial 3036 en TMR0
    btg LATB, 5 ;basculamos ((PORTB) and 0FFh), 5, a donde esta el LED
    retfie
