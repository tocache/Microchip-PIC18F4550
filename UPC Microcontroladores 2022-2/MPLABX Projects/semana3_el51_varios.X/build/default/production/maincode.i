...
camote:
    ...
    ORG 000020H
configuro:
    bcf TRISC, 0 ;((PORTC) and 0FFh), 0, a como salida
    bcf INTCON2, 7 ;((INTCON2) and 0FFh), 7, a=0 res weak pull-up en RB activadas

lazo:
    btfsc PORTB, 1 ;Pregunto si presionaste ((PORTB) and 0FFh), 1, a
    goto lazo ;Falso: No presionaste regresa a preguntar
    btg LATC, 0 ;Verdad: Basculo ((PORTC) and 0FFh), 0, a (enciendo o apago el LED)
otro:
    btfss PORTB, 1 ;Pregunto si dejaste de presionar ((PORTB) and 0FFh), 1, a
    goto otro ;Falso, sigues presionando ((PORTB) and 0FFh), 1, a regresa a preguntar
