...
configuro:
    ...
    bcf TRISD, 0 ;((PORTD) and 0FFh), 0, a como salida
    movlw 0C0H
    movwf T0CON ;Timer0 ON, 1:2 PSC, Fosc/4, 8bit
    bsf LATD, 0 ;Encendemos LED en ((PORTD) and 0FFh), 0, a

inicio:
    movlw 6
    movwf TMR0L ;Carga de cuenta inicial 6 en Timer0
otro:
    btfss INTCON, 2 ;Preguntamos si se desbordo Timer0 (((INTCON) and 0FFh), 2, a=1)
    goto otro ;falso, vuelve a preguntar
    btg LATC, 0 ;verdad, bascula ((PORTC) and 0FFh), 0, a
