...
configuro:
    ...
    bcf TRISE, 0 ;((PORTE) and 0FFh), 0, a como salida
    movlw 0C0H
    movwf T0CON ;Timer0 ON, Fosc/4, 8bit, PSC 1:2
    bsf LATC, 0 ;((PORTC) and 0FFh), 0, a=1

inicio:
    movlw 6
    movwf TMR0L ;Carga de cuenta inicial a Timer0
    btfss INTCON, 2 ;Pregunto si ((INTCON) and 0FFh), 2, a=1 (si se desbordo el Timer0)
    bra $-2 ;Retorna a preguntar
    bcf INTCON, 2 ;Bajamos bandera ((INTCON) and 0FFh), 2, a
    btg LATE, 0 ;Bascula salida ((PORTE) and 0FFh), 0, a
