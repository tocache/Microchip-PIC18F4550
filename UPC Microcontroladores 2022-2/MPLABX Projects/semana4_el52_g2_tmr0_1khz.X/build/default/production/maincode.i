...
configuro:
    ...
    bsf LATC, 0 ;((PORTC) and 0FFh), 0, a=1 encendemos el LED en ((PORTC) and 0FFh), 0, a
inicio:
    movlw 6
    movwf TMR0L ;carga de cuenta inicial a Timer0
    btfss INTCON, 2 ;Pregunto si ((INTCON) and 0FFh), 2, a=1 o si se desbordo Timer0
    bra $-2 ;falso, vuelve a preguntar
    bcf INTCON, 2 ;verdad, bajo la bandera
    btg LATE, 0 ;basculo salida ((PORTC) and 0FFh), 0, a
