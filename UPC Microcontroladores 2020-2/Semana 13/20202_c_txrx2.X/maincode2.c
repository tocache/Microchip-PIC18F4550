...
unsigned char menu6[] = {"i - Ingresa cadena                   "};
unsigned char rmenu6[] = {"No debe de pasar de 20 caracteres   "};
...
unsigned char opt_i[] = {"El mensaje ingresado fue:          "};
...
void __interrupt(high_priority) RC_Isr(void){
    ...
    if(indicador ==0){
        ...
        //rutina para recepcionar la cadena y detener el ingreso detectando el enter
        if(RCREG == 0x0D){
            //termino el ingreso de la cadena y lo muestro en el LCD
            EUSART_enviacadena(opt_i,37);
            EUSART_siguientelinea();        
            EUSART_enviacadena(ingreso,20);
            EUSART_siguientelinea(); 
            indicador = 0;
        }
        else{
            ingreso[indice] = RCREG;
            indice++;
        }
    }
}